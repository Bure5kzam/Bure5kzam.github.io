...
[vscode 설정파일 설정하기]({% post_url 2023-11-14-vscode-setting-json %})

[vscode 설정파일 세부 종류]({% post_url 2023-10-17-vscode-setting-scope %})
...
[vscode 확장 프로그램 설정하기]({% post_url 2024-01-14-vscode-extension-configuration %})
[vscode 확장 프로그램 오프라인에서 구성하기]({% post_url 2023-10-23-오프라인 환경에서 vscode server 익스텐션 구성하기 %})
...
sync는 설정을 온라인이나 계정에 저장해둿다가 복원하는 기능이다.

포맷한 기기를 프로비저닝할 때 개발환경을 바로 불러올 수 있어서 좋다.

Github gist으로 온라인 복원도 가능하고, 로컬에 한번 저장하면 오프라인에서도 사용할 수 있다.

## snippet : 코드 조각 삽입하기

스니펫을 설정하면 반복해서 사용하는 코드조각을 접두사를 입력해서 삽입할 수 있다.

스니펫을 구성하려면
1. 언어별 스니펫 파일 생성
2. 삽입할 스니펫의 내용과 불러올 접두사, 삽입 위 커서의 위치 정도를 정의한다.

끝!

### 관련 포스트

[vscode snippet 예제]({% post_url 2023-11-17-vscode-snippet-example %})

[vscode snippet 설정 파일 문법]({% post_url 2023-11-18-vscode-snippet-syntax %})

[vscode snippet 설정 파일 적용 범위]({% post_url 2023-11-19-vscode-snippet-scope %})
