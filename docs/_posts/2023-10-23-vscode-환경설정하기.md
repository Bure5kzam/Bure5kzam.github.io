...
> Reference : 23-10-23 [Vscode  Reference](https://code.visualstudio.com/docs)
...
상세한 설정에 앞서 실행된 vscode 인스턴스에 영향을 주는 것이 얼마나 많은지 아는 것이 좋다.

vscode 설정 내용은 다양한 위치에 Json 포맷으로 저장되어 있으며, 좁은 범위에 적용되는 파일이 우선 적용된다.

일반적인 설정은 다음과 같다.

1.  `defaultSetting.json`  모든 인스턴스에 적용되는 설정. MS에서 권장하는 구성이다.
	1. IDE에서 열어 볼 수 있으나 read-only가 적용돼있다.
	2. [vscode 레퍼런스]( [Ms docs](https://code.visualstudio.com/docs/getstarted/settings#_default-settings)에서도 볼수있음.
2. `~/.config/Code/User/settings.json`  OS에 로그인한 유저에 적용되는 설정. 
	1. [vscode 레퍼런스, User & workspace setting](https://code.visualstudio.com/docs/getstarted/settings)
3. `${workspaceFolder}/.vscode/settings.json` 워크스페이스에 적용되는 설정.
	1. `workspaceFolder`는 현재 열린 프로젝트 루트 경로로 치환되는 vscode 자체 변수다.
	2. [vscode 레퍼런스, Workspace setting](https://code.visualstudio.com/docs/getstarted/settings#_workspace-settings)
	3. vscode 자체 변수가 궁금하다면 [vscode 레퍼런스, Variable referecne](https://code.visualstudio.com/docs/editor/variables-reference#_predefined-variables) 참고

요약하면 워크스페이스 설정 >> 유저 설정 >> 기본 설정 순으로 우선 적용된다.

> * 스코프라는 표현이 맞는지 모르겠다. 레퍼런스에서 공식적으로 사용하는 표현은 아니지만 설명한 파일들이 서로 적용되는 범위가 달라서 그렇게 표현했다.
...
앞서 말했듯 설정 내용은 Json 파일로 관리하기 때문에, 수정하려는 속성의 키값을 도큐먼트에서 찾아서 수정해서 설정할 수 있다.

하지만 IDE 자체 편집 화면을 화면을 제공하므로 임의 수정이 부담스럽다면 UI로 하면 된다. (내 버전에서는 File >> Settings나 사이드바 하단의 톱니 버튼을 눌러 수정할 수 있다.)
...
위에서 다양한 적용 범위를 가진 `settings.json` 파일을 설명했다.

`settings.json` 은 배경색, 단축키 처럼 편집기 수준의 설정을 담은 파일이다.

디버깅, 실행, 자동완성, 컴파일처럼 개발에 필요한 기능은 추가로 설정해줘야한다.

`user setting`과 `workspace setting` 설정 경로에 관련 파일을 추가해서 적용할 수 있다.
...
[Vscode 레퍼런스, Programming Launguage](https://code.visualstudio.com/docs/languages/overview)
...
[Vscode 레퍼런스 : Profile](https://code.visualstudio.com/docs/editor/profiles) 에는 다음과 같은 설명이 있다.
